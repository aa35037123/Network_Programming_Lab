...
def packet_callback(packet):
    ...
    try:
        ...
        # print()
        ip_option = packet[IP].options
        udp_payload = packet[2].load.decode()
        print(f'udp payload: {udp_payload}')
        print(f'length of udp payload: {len(udp_payload)}')
        print(f'ip option: {ip_option}')
        # if packet.haslayer(UDP) and \
        #     (packet[UDP].sport == PORT or packet[UDP].dport == PORT):
        #     captured_packets.append(packet)
        packet_dict[udp_payload] = packet
        # capture_packets.append(packet)
        # if udp_payload[:4] == 'hello':
        #     packet_dict['hello'] = packet
        # elif udp_payload[:1] == 'OK':
        #     packet_dict['OK'] = packet
        # elif udp_payload[:4] == 'chals':
        #     packet_dict['chals'] == packet
        # ## seq
        # else:
        #     seq_num = int(udp_payload[4:8])
        #     seq_array[seq_num] = ip_option + udp_payload
        #     packet_dict[f'{seq_num}'] = packet
        #     flag = udp_payload[10:]
        #     if flag == 'BEGIN FLAG':
        #         begin_flag = True
        #         begin_seq = seq_num
